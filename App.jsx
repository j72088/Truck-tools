...
const formatMoney = (value) => {
  if (Number.isNaN(value) || value === '' || value === null) return '$0.00'
  return new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(Number(value) || 0)
}

const readHistory = () => {
  try {
    const raw = localStorage.getItem('trucker-tools-history')
...
export default function App() {
  const [trip, setTrip] = useState({ revenue: '', miles: '', fuelCost: '', otherCosts: '' })
  const [cpm, setCpm] = useState({ totalCosts: '', miles: '' })
  const [pay, setPay] = useState({ miles: '', rate: '', deductions: '' })
  const [route, setRoute] = useState({ miles: '', mpg: '', fuelPrice: '', tolls: '', extras: '' })
  ...
  const persist = (entry) => {
    ...
    setHistory(next)
    localStorage.setItem('trucker-tools-history', JSON.stringify(next))
  }
